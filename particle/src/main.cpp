...
typedef RPCServer<std::string, std::string> ParticleServerBase;

class ParticleServer : public ParticleServerBase
{
public:
    ParticleServer(const std::string &svr_id)
        : ParticleServerBase(svr_id)
    {
	    mosquitto_lib_init();
    }
...
public:
    ...
    virtual void on_message(const struct mosquitto_message *message)
    {
        ...
        const std::string msg((const char*)(message->payload));
        ...
        int mid = 0;
        std::string response = msg + msg + msg;        
        int res = publish(&mid, 
                          publish_topic.c_str(),
                          response.size(),
                          response.c_str());
